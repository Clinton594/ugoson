...
$.fn.extend(
{
	createHeaderRow: function(field,order,classs)
	{
        ...
        var nDiv=$('<div>').css({'float':'left','margin-top':'2.2rem'}).addClass('row col s11').appendTo($(this));
		var counter  = 0;
		$.each(field, function(j, col)
		{
		  var thisclass = (classs[j] == '') ? 'col s'+num : classs[j];
          if (order[j] != 'hidden') {
            nDiv.append($("<div>").css({'margin-top':'0px','font-weight':'bold'}).addClass(thisclass).html(col+ " "));
          }
      counter++;
		});
	}
...
$.fn.extend(
{
	createRow: function(field,order,ext,classs)
	{
		var data = hidden(order), _hidden = data.hidden, total = data.count, _notHidden = parseInt(total) - parseInt(_hidden);
		var num=Math.floor(12/_notHidden);
        var nDiv=$('<div>').addClass('row col s11').appendTo($(this));
		var counter = 0;
		if(ext.length){$('<div>').addClass('extcol').appendTo($(this)).optionBox(ext,field.i);}
		$.each(field.c, function(j, col)
		{
			...
			if(order[j] !== 'hidden'){
			    if(order[j]=="img")
    			{
						nDiv.append($("<div>").addClass(thisclass).append($("<img>").attr({src:col}).css({height:"20px",margin: "0 auto",float: "none",display: "block"})));
    			} else if (order[j]=="span")
    			{
    				nDiv.append($("<span>").text(col).addClass(thisclass).css({'font-size':'11px','font-style':'italic'}));
    			}else if (order[j]=="bold")
    			{
    				nDiv.append($("<b>").addClass(thisclass).text(col));
    			}
			}
      counter++;
		});
	...
	optionBox: function(field,id)
	{
		if(field !=null && field !=undefined)
		{
			...
			$.each(field,function(i,col)
			{
				...
				var ed2 = $('<span>').attr({title:col.page_title, "data-pageid":col.pageid,'data-id':id,'data-open':opener}).append(
				$('<i>').addClass('editors').append($('<img>').addClass('material-icons tiny').attr({'src':'icons/'+col.icon+'.svg'}))).click(function(e){
					e.stopPropagation();
...
function editPicture(a,b){
	...
	editModal(a,function(o,c){
		...
		$.post("process_generic.php",c).done( function(rspText,status,xHr)
			...
			}).fail(function(e){M.toast({html: "Error in sending message",displayLength: 1000,classes: ""});
			});
...
$.fn.extend({
	questionBox:function(q,f, mini = false){
		...
		$('#_question_dialog').openModal({
		  onOpenEnd: function() {if(mini && div){div.addClass("small")} } // Callback for Modal open
		});
	}
});
...
$.fn.extend({leanModal: function (obj) {
	let object = obj || {};
	$(this).modal(object)
}})
$.fn.extend({ openModal  : function (obj) {
	let object = obj || {};
	$(this).modal(object);
	$(this).modal("open");
}})
$.fn.extend({closeModal : function () {
	$(this).modal("close");
}})
