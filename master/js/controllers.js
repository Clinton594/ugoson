...
class Site {
	constructor() {
		...
		getCookie('siteData', function(rear){
			if(rear.length > 10){//cookie string is not empty
				let uri = rear.split(',');
				thx.domain = uri[0];
				thx.backend = uri[0]+uri[1];
				thx.absolute_filepath = uri[2];
				thx.process_url = `${thx.backend}process/controllers`;
...
$.fn.extend({
	socialShare:function (obj){
		let read = false;
		let $this = $(this);
		if(typeof(obj) !== "object"){	obj = {};	read = true;		}
		if($(this).data("name")){
			...
			$(this).click(function (){
				...
				let link = ""; let client = ""; let resolution = 'width=626,height=436';
				let dd = {};
				if(read === false){//Share from card in list view
					$.post(site.process_url, obj, function(data){
						let dd = isJson(data);
						if(!dd){
							console.log(data);
				...
				}else{dd.url = window.location; dd.text = ""}
				if(dd != undefined || read){
					switch (socialMedia) {
						case 'fb': link = 'https://www.facebook.com/sharer/sharer.php?u='+dd.url;client = 'facebook-share-dialog';break;
						case 'tw': link = 'https://twitter.com/intent/tweet?text='+title+'&url='+dd.url;client = 'twitter-share-button';break;
						case 'li': link = 'https://www.linkedin.com/sharing/share-offsite/?url='+dd.url;break;
						case 'wh': link = 'whatsapp://send?text='+dd.url;break;
						default: link = "undefined";client = 'facebook-share-dialog';break;
					}
					window.open(link, client, resolution); return false;
				}
			})
		}else{
...
$.fn.extend({
	...
	*/
	...
	loginForm: function(object, callback) {
		...
		if(child !== 'form'){alert("This element must be a form element !!!");}
		if(!object.formName){
			alert("loginForm plugin requires 'formName' property to initialize. This form formName is the name of the param to get table information from. See 'admin_signin' param or documention for more details");
			return ;
		}else{
			object.case 			= 1.6;
	...
	*/
	forgoTPassword: function(a) {
		...
		if(child !== 'form'){
			...
			alert("forgoTPassword plugin requires 'formName' property to initialize. It is the name of the param from which to extract the member's table and columns.");
		}else{
			$this[0].param = a;
					...
					if(forgotbutton.length !== 0){
						forgotbutton.attr({href:"javascript:;"}).click(function(){
							...
							if(user_identifier){
								...
								$.post(site.process_url, Forgot, function(res){//after sending email
									...
									if(data.status){
										...
										setTimeout(function () {//Disable the generated code after 10mins
											$.post(site.process_url, {case:1.35});
										},60 * 10 * 1000);
										$this.swapDiv(fpass);
									}else{
										console.log(res);
								...
								let mapp = {email:"Email", text:"Email or Username", "":"Information in the input field", tel:"11 digit Phone Number", number:"Information in the input field"}
								mapp[text] == mapp[text] || input.attr("name");
								toast("Please enter your "+mapp[text], "indigo");
							}
						});
						$this.parent().append($('<div>').attr({'id':'forgot-panel','data-toggle':'login-panel'}).css({'display':'none','opacity':'0','max-width':'400px','border': 'solid 1px #ccc','border-radius': '5px','padding': '15px',margin:'auto !important'}).append(
							$('<form>').attr({'action':'javascript:;',onsubmit:'return(false)'}).append($('<h3>').css({'padding':'15px'}).addClass('mt-1 name text-center').text('Forgot Password')).append($('<p>').css({'line-height': '20px','text-align': 'center','padding': '15px',width:'100%'}).css({'line-height':'15px'}).append($('<span>').addClass('reset-notice')).append($('<span>').text('. Get the token and change your password, check spamBox too.'))).append($('<div>').css({width:'100%', padding:'15px'}).append($('<div>').addClass('ml-0 mr-0 search-container log-input').append($('<input>').addClass('form-control ml-0').attr({'placeholder':'Enter the TOKEN code','type':'number', 'id':'token', 'name':'token', 'required':'required' }).css({ 'border-radius':'3px'})))).append($('<div>').css({width:'100%', padding:'15px'}).append($('<div>').addClass('ml-0 mr-0 search-container log-input').append($('<input>').addClass('form-control ml-0').attr({'placeholder':'Enter your new password','type':'password', 'id':'password', 'name':'password', 'required':'required' }).css({ 'border-radius':'3px'})))).append($('<div>').css({width:'100%', padding:'15px'}).append($('<div>').addClass('ml-0 mr-0 search-container log-input').append($('<input>').addClass('form-control ml-0').attr({'placeholder':'Confirm your new password','type':'password', 'id':'password2', 'name':'password2', 'required':'required' }).css({ 'border-radius':'3px'})).append($('<input>').addClass('form-control ml-0 user_id').attr({type:"hidden", name:"id"}).css({ 'border-radius':'3px'})))).append($('<div>').css({width:'100%',padding:'15px'}).append($('<a>').addClass('btn-error btn').attr({'href':'javascript:;'}).click(function(){$('#forgot-panel').swapDiv($this);}).text('Cancel')).append($('<button>').addClass('button submit success place-right btn-primary btn float-right').text('Reset')))));
							let reset_data = $.extend({}, a);
							reset_data.case = 1.4;
							$('#forgot-panel').find("form").submitForm(reset_data, null, function ($data) {
								if($data.status == 1){
									console.log($data);
...
$.fn.extend({// Update cart in rear
  updateCart: function(data, callback){
    $.post(site.process_url,{data:data, case:1.5}, (response)=>{
      if(typeof(callback) == "function"){
				callback(response);
...
$.fn.extend({
	submitForm: function(parameters, callbefore, callback) {
		...
		if(!domName || domName.toLowerCase() !== 'form'){
			alert("This is not a form element !!!"); return ;
		}else if(form.find("button.submit").length == 0){
			alert("add classname 'submit' to the submit button"); return ;
		}else{
			...
			form.submit(function () {
				...
				if(form[0].checkValidity() === true){
					...
					if(typeof(callbefore) === "function"){
						...
						if(formData.error){toast(formData.error, "red", formData.duration);return false;}
					}
					...
					if(proceed === true && button[0].go === true && formData){
						...
						button[0].go = false;
						let process_url = formData.filter((obj)=>{return obj.name == "process_url";});
						process_url = process_url[0] ? process_url[0].value : parameters.process_url;
						process_url = process_url || site.process_url;

						// Default case
						let kase = formData.filter((obj)=>{return obj.name == "case";});
						if(!kase.length)formData.push({name:'case', value:1.7});
...
$.fn.extend({
	initComments: function(paramters){
		...
		$comment_container.append(
			$("<section>").attr({id:"comments-section"})
			.append($("<div>").addClass("row")
				.append($("<div>").addClass("comment-list row"))
			)
			.append($("<div>").addClass("comment-form row")
				.append( $("<form>")
					.append($("<div>").addClass("form-control").append(
						$("<input>").attr({name:"user_name",type:"text",required:true}))
					)
					.append($("<div>").addClass("form-control").append(
						$("<input>").attr({name:"message",type:"text",required:true}))
					)
					.append(
						$("<div>").addClass("form-control").append(
							$("<button>").addClass("submit").attr({type:"submit"}).text("Post")
						)
					)
				)
			)
...
$.fn.extend({
	loadComments: function(paramters){
		let $post_id = $(this).parent().data('id');
		let $comment_container = $(this);
		let comment_list = $comment_container.find(".comment-list")
		paramters.intval = paramters.intval || 10*60;
		$comment_container[0].param = paramters;
		if($post_id){
			setTimeout(function() {
				$.post(site.process_url, {case:4, post_id:$post_id}, function (response) {
					let $data = isJson(response);
					if($data){
						comment_list.empty();
						for(let i in $data){
							comment_list.oneComment($data[i]);
						}
					}
				})
				//First Load
				if($comment_container.prop("interval") == undefined){
					var interval = setInterval(function () {
						$comment_container.loadComments(paramters);
					}, paramters.interval);
					$comment_container[0].interval = interval;
					//Check if paramters exists and it has ability to submit a comment
					if(paramters && paramters.submitForm){
						let formparamters = $.extend({},paramters.submitForm);
						delete paramters.submitForm.callbefore;
						$comment_container.find("form").submitForm(paramters.submitForm, formparamters.callbefore, function ($resdata) {
							if($resdata.success){
								$resdata.time = "now";
								comment_list.oneComment($resdata);
								$comment_container.find("form input[name=user_name]").attr({readonly:true});
								$comment_container.find("form [name=message]").val("");
								if(typeof(paramters.submitForm.callback) == "function"){
									let callback = paramters.submitForm.callback;
									callback($resdata);
								}
							}
						});
			...
			alert("Add the post id to this element as data-id")
		}
	}
})
$.fn.extend({
	oneComment: function(obj){
		let paramters = $(this).closest("#comments-section").prop("param");
		let mine = paramters.user_id == obj.user_id ? "me": "";
		let comment = $('<li>').addClass('comment '+mine).append($('<div>').addClass('vcard').append($('<img>').attr({'src':site.backend+'images/c_icon.png','alt':'Image placeholder'}))).append($('<div>').addClass('comment-body').append($('<h3>').addClass('com-name').text(obj.user_name)).append($('<div>').addClass('meta').text(obj.time)).append($('<p>').text(obj.message)));
		$(this).append(comment);
	}
})
...
$.fn.extend({
	updateViews: function (callback) {
		...
		if($post_id){
			$.post(`${site.process}files`, {case:"noOfViews", folder_name:$folder_name, id:$post_id}, function (response) {
				let $data = isJson(response);
...
$.fn.extend({
	...
	formdata: function(key, value){
		...
		if(!domName || domName.toLowerCase() !== 'form'){toast("This is not a form element !!!"); return ;}
		let formdata = [];
		// Creates a default empty formdata array if there isnt any one existing before or to to empty an exitsting formdata
		if(!form.prop("formdata") || key === false){ form[0].formdata = formdata;}
		if(key){
			formdata = form.prop("formdata");
			// Remove every existing key from the formdata array
			if(value){
				formdata.forEach((item, i) => {if(item.name == key)formdata.splice(i, 1);});
				...
				formdata = form.prop("formdata") || [];
				formdata.forEach((item, i) => {if(item.name == key)value = item.value;});
				return value;
			}
		}else{
			formdata = form.prop("formdata") || [];
...
$.fn.extend({
	startLoader : function(disable_form, size = 16){
		...
		let $this 	= $(this);
		if($this){
			$this.append($("<img>").addClass("_loader").attr({src:site.backend+"icons/rotate_right.svg", disabled:true}).css({width:size}));
...
function getCookie(d, callback) {
  ...
  var _A = d_.split(';'), r = {}, c = 0;
	var d = d.split(",");
	d = d.map(function (pa) {return pa.trim();});
...
if(typeof(toast) !== "function"){
	function toast(msg, color="green", time){
		time = time || 2000;
		var ts = $('<div>').appendTo('body').css({overflow: 'hidden',display: 'block',position: 'fixed','z-index': '800001','top': '20%',left: '4%','background-color':color,'border-radius': '2px','text-align': 'center'}).attr({id:'toast-container_'}).append($('<div>').css ({
			'background-color':color,opacity:'1','float': 'left','padding': '5px 10px','text-align': 'center','line-height': '30px', 'margin-top':'0px','font-size': '15px','color': 'white','font-family': "'Raleway', sans-serif"
		}).addClass('toast_').text(msg));
		$(ts).animate({top:0},2000);
		setTimeout(function(){$(ts).remove()}, time)
	}
}
...
$.fn.extend({
	slideInFromEdge : function (callback) {
		let slide = $("<div>").css({position:"fixed",left:0,right:0,bottom:0,top:0,backgroundColor:"rgb(21 19 19 / 74%)", zIndex:1000}).click(function () { $(this).closeSlider();});
		$(this).append(
			slide.append(
				$("<div>").addClass("slideInFromEdge").css({width:0,maxWidth:480,top:0,right:0,bottom:0,position:"fixed",backgroundColor:"#292d35",boxShadow:"-10px 0px 10px 1px #1f1e1e3b"}).animate({
					width:"100%"
				}, "slow", function () {
					...
					$("<p>").append(
						$("<img>").attr({src:`${site.backend}icons/clear.svg`}).css({padding:15, cursor:"pointer"}).click(function () {
							$(this).closest(".slideInFromEdge").parent().closeSlider();
						})
					)
				).append(
					$("<section>").addClass("slide-content").css({width:"100%", padding:"0 20px"})
				)
			)
		);
		if(typeof(callback) == "function"){ callback(slide.find("section"));}

	},
	closeSlider: function (callback) {
		let slide = $(this);
		slide.children().animate({
			width:0
		}, 1000).children().fadeOut();
		setTimeout(()=>{
			slide.remove();
			if(typeof(callback) == "function"){
				callback();
