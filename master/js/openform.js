...
function formInitialize(p, t, key = 1) {
  ...
  $("#" + a).find("form").each(function() {
    ...
    form.find('#uploadPic' + a).click(function() {
      $(this).uploadDialog({
...
function loadOpen(a) {
  ...
  $.post(param, function(res) {
    ...
    if (result.row == undefined || result.row.length == 0) {
      var ehd = $('<div>').addClass('center').appendTo(currPage).css({
        'width': '100%',
        'margin-top': '5%'
      });
      let range = $(`#${a}`).find(".dateFilter").length ? "within the selected date range":"";
      if ($('#_searchBox' + a).val() == '') eText = "No " + pagetitle + ` found to display ${range}, click the "( + )" button below`;
    ...
    var hd = $('<div>').addClass('collection-header left capitalize').appendTo(currPage).css({
      'width': '100%'
    });
    var nCh = $('<label>').css({'float': 'left', 'margin-top': '2.2rem', 'margin-left': '10px' }).appendTo(hd);
    ...
    $('<span>').appendTo(nCh).attr({
      'for': '_selectAll' + a
    }).click(function(e) {
      e.stopPropagation();
    });
    //$('<h3>').html(pagetitle).appendTo(hd).css({'float':'left'});
    $(hd).createHeaderRow(result.desc, result.fmt, result.cls);
    var opener = currPage.data('open') || 'swipe';
    var $recordstart = (page - 1) * limit;
    $("#listCount"+a).val(result.total+" Item(s)");
    $.each(result.row, function(i, field) {
      $recordstart++;
      var nD = $("<a>").addClass("oRow collection-item capitalize").css({
        float: 'left',
        position: 'relative',
        width: '100%'
      }).appendTo(currPage).attr({
        ...
        $(this).parent().find('a.collection-item').each(function() {
      ...
      var nCs = $('<label>').css({'float': 'left'}).appendTo(nD);
      $('<input>').attr({
        'type': 'checkbox',
        'id': 'cbx_' + field.i + a,
        'value': field.i
      }).appendTo(nCs).click(function(e) {
        e.stopPropagation();
      }).addClass('cbx filled-in');
      $('<span>').css({
        'float': 'left'
      }).text($recordstart + '. ').appendTo(nD);
      $('<span>').appendTo(nCs).attr({
        'for': 'cbx_' + field.i + a
      }).click(function(e) {
        e.stopPropagation();
      });
  ...
  $.getJSON(process_url+"?pageType=" + pagetype + filter_param, function(result) {
    ...
    $.each(result, function(i, field) {
      ...
      $.each(field, function(j, col) {
        ...
        cont.find('.displaypicture > img.' + j ).attr({
          src: col
        })
...
function loadSelection(ths, a, formpage, form_view) {
  ...
  $.get(process_url+"?pageType=" + pagetype + filter_param, function(result) {
    ...
    if (form_view == "extension"){
      ...
      // Select options
      form.find("select[onchange]").change();
      form.find("select[multiple]").each(function() {
        // let dx = this;
        // setTimeout(()=>{
...
function printForm(a, thx, id = null) {
  ...
  let pageType = $("#page_type"+a).attr("value");
