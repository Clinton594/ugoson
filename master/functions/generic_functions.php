...
function array_serialize($array = array()){
	...
	if(!empty($array)){
		foreach($array as $key => $val){
			$key = trim($key); $val = trim(urlencode($val));
			$build[] = "$key=$val";
		}
	}
  ...
  if(!is_dir($dir)){    mkdir($dir, 0777, true);  }
	...
	if(!empty($data)){
	    ...
	    if(fwrite($handle, $data)){
	      ...
	      if($returnData === true){return($data);}else{return(true);}
	    }else return(false);
			fclose($handle);
		}
  }else return(false);
...
function clear_files_longer_than($days = 30, $dir){
	if(is_dir($dir)){
		...
		while (false !== ($file = readdir($handle))) {
			...
			if($diff > $days && !is_dir($thisfile)){unlink($thisfile);continue;}
		}
	}
	clearstatcache();
}
...
function see($arr, $stop = true){
  if(gettype($arr) == 'array' || gettype($arr) == 'object'){
    echo "<pre>";
     ...
     if($stop !== true){print("$arr");}else{die("$arr");}
    }
  }
...
function get_in_clause($string = '', $clause){
  if(!empty($string)){
    $startPos = stripos($string, $clause);
    $endPos = stripos($string, ')', $startPos)+1;
