			 ...
			 if($file=='.' || $file=='..' ||  basename($file)=='tbn') continue;
			  	if(is_dir($dir . $file)){//folder
					...
					if(checkPictureFormat($file)){
						...
						if(file_exists($icon))$in[$count]['icon']= http_filepath($icon);
						else $in[$count]['icon']= http_filepath($dir . $file);
					}
					$in[$count]['name']= basename($file);
...
function save_base64_file($base64_string, $output_file) {
	...
	if(!is_file($icon) && checkPictureFormat($output_file)){
		resizePicture($output_file, $icon, 128, 128);
		if(file_exists($icon))$output_file = $icon;
	}
	return $output_file;
}
...
function thumbnail($filename){
	$filename = str_replace("/tbn/", "/", $filename);
	$thumbnail = dirname($filename)."/tbn/".basename($filename);
