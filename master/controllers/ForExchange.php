...
class ForExchange{
  ...
  public function cryptoRates($coins = [], $createOnly = false) {
    ...
    if(file_exists($file) && $createOnly !== true && (round(($today - filemtime($file))/60) < 300000)){
       $rates = json_decode(_readFile($file));
    }else{
      $key = $this::$generic::$coinlib["API_KEY"];
       $rates = file_get_contents("https://coinlib.io/api/v1/coinlist?key={$key}&pref=USDT&page=1&order=volume_desc");
       $rates = isJson($rates);
       $rates = $rates->coins;
  ...
  public function currencyGraph($coins = [], $createOnly = false) {
    ...
    $response = [];
    if(!is_dir($dir)){ mkdir($dir, 0777, true);}
    if(file_exists($file) && $createOnly !== true && (round(($today - filemtime($file))/(3600*24)) < 3)){
       $rates = json_decode(_readFile($file));
    }else{
       $rates = file_get_contents("https://coinlib.io/searchable_items_json?v=109987&json");
  ...
  public function toBTC($currency_amount = 1){
    $response = 0;
    $response = curl_post("https://blockchain.info/tobtc?currency={$this->base}&value={$currency_amount}");
