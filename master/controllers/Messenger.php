...
class Messenger{
  ...
  public function sendMail($post){
    if(gettype($post) !== 'object'){$post = (object) $post;}

    if(isset($data->process_url)){//Send online
      ...
      foreach($params as $field){if(!isset($post->{$field})){return("$field field is not defined for sending email parameters");}}
      ...
      if(!in_array(self::$generic->getServer(), self::$generic->getLocalServers())){
        ...
        $Mail->AddReplyTo($post->from, "RE: $subject");
        $Mail->From     = $post->from;
   ...
   public function sendGridMail($post){
