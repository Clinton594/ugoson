...
class Generic extends DBCred{
  ...
  public function __construct(){
    global $_SERVER;
    parent::$server  =  $_SERVER['SERVER_NAME'];
  }
  ...
  public function connect($buildConstants = true){
    if(!$this->isConnected){
      ...
      $connect = (array_search(self::$server, self::$local_servers) !==false ) ?
      [parent::$local_server, parent::$local_user, parent::$local_password, parent::$local_db] :
      [parent::$local_server, parent::$online_user, parent::$online_password, parent::$online_db];

      $db_handle = new MySQLi($connect[0],$connect[1],$connect[2],$connect[3]);
  ...
  function buildAllConstants($createOnly = false){
    ...
    if(file_exists($file) && $createOnly !== true &&  (round(($today - filemtime($file))/(3600*24)) < 7)){
      ...
      $db_name = (array_search(self::$server, self::$local_servers) !==false ) ? parent::$local_db : parent::$online_db;
      $fields = [];
      $sql = "SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '{$db_name}'";
      $query = self::$mydb->query($sql) or die();
  ...
  public function buildQuery($post){
    ...
    foreach ($post as $key => $value) {
      ...
      $excludes = ["now()"];
      $v=self::$mydb->real_escape_string($value);
      ...
      if(!empty($searchCols)){
        ...
        if(isset($filters["keyword"])){
          ...
          foreach ($searchCols as $col) {
            if(array_search($col, $fields) !== false){$search[] = "$col LIKE '%$keyword%'";}
          }
      ...
      if(!empty($filter)){
        ...
        foreach($arr as $fltr){
          ...
          foreach ($explosives as $xk => $xv) {
            if(gettype(stripos($fltr, $xv)) !== 'boolean'){ $explode_key = $xv; break; }
          }
          if(gettype(stripos($fltr, $explode_key)) !== 'boolean'){//if contains an explosive
            ...
            $filters[] = get_in_clause($filter, $fltr);
          }
        }
  ...
  public function getFromTable($table, $filter = '', $page = 1, $pagesize = 1, $sort = "", $search = false, $cols = "*"){
    $content = [];
    ...
    $content = [];
      ...
      if(self::$server !== "localhost" && array_search(self::$server, self::$local_servers) !==false ){
        $row = array_map(function ($value){
          return str_replace("localhost", self::$server, $value);
        }, $row);
      }
			$row = (object) $row;
			$content[] = $row;
    ...
    if(!empty($post->formName)){
      ...
      if(count((array)$param) && isset($param->table)){
        ...
        */
        ...
        $result = $response = self::insert($post, $param);
  ...
  public function insert($post, $param){
    ...
    $response = new stdClass;
    $build    = array();
    $fields   = self::getTableFields($param->table);
    $proceed  = 0;
    $response->status = 0;
    foreach ($post as $key => $value) {
    ...
    if(empty($post->{$param->primary_key}) || (!empty($post->submitType) && $post->submitType === "insert")){
      ...
      if(!empty($param->unique_key)){
        if(!empty($post->{$param->unique_key})){
          ...
          $unique = self::getFromTable($param->table, "$unique_key=$unique_value",1,1,"",false,[$param->primary_key]);
          if(!empty($unique[0])){
    ...
    if($proceed === 1){
      ...
      $id = !empty(self::$mydb->insert_id) ? self::$mydb->insert_id : $post->{$param->primary_key};
      $response->status = 1;
      ...
      if(!empty($param->logging) && gettype($param->logging) == "object"){
        ...
        $user   = $auth->user();
  			...
  			$descr	=	"{$user->name_col} $action $descr {$param->page_title}"; $location_id = "";
  ...
  public function company(){
    ...
    if(empty(self::$company) || self::$company == null){
      if($this->has_db && $this->isConnected){
        $query = self::$mydb->query("SELECT * FROM company_info WHERE id='1'");
